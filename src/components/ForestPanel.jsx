...
import {
  FOREST_CSS, ambientSpans, defsFor, drawBackdrop, drawLandmark, drawTree,
  durationMin, el, fmtDate, fmtMin, foliageFill, hillY, jitter, monthKey, monthLabel, parseTs,
  seasonClass, sessionCategory, speciesFor,
} from '../lib/forestDraw'
...
export default function ForestPanel() {
  ...
  // Two forests over the same sessions: the project ecosystems (default)
  // and the original month groves.
  ...
  const toggle = (
    <div className="flex items-center gap-3 mb-4 flex-wrap">
      <div className="flex gap-1 rounded-lg bg-slate-200/70 dark:bg-slate-800 p-1">
        {[['projects', 'Projects'], ['months', 'Months']].map(([id, label]) => (
          <button
            key={id}
            onClick={() => setView(id)}
      ...
      <span className="text-xs text-slate-400 dark:text-slate-500">
        {view === 'projects' ? 'One tree per project, kept alive by its sessions' : 'One tree per session, planted by month'}
      </span>
    </div>
  )
  const shell = (inner) => (
